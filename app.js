...
function drawboard(difficulty = "easy") {
        ...
        tds.forEach((td) => {
          for (let y = 0; y < rowIndex.length; y++) {
            for (let x = 0; x < columnIndex.length; x++) {
              if (
                td.parentNode.rowIndex === rowIndex[y] &&
                td.cellIndex === columnIndex[x]
              ) {
      ...
      tds.forEach((cell) => {
        for (let y = 0; y < rowArray.length; y++) {
          for (let x = 0; x < columnArray.length; x++) {
            if (
              cell.parentNode.rowIndex === rowArray[y] &&
              cell.cellIndex === columnArray[x]
            ) {
          ...
          if (td._cell.count === 0) {
            ...
            function scanningSurroundingCells(td) {
              td._cell.surroundingCells.forEach((surroundingCell, index) => {
                if (
                  surroundingCell._cell.bomb === false &&
                  surroundingCell._cell.viewed === false &&
                  surroundingCell._cell.marked === false
                ) {
  ...
  // counting number of bombs to have the right number of "marks/flags"
  function countingFlags() {
    tds.forEach((td) => {
      if (td._cell.bomb === true) {
        mark++;
      }
